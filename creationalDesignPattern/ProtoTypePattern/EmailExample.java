...
/**
 * This code demonstrates the Prototype Design Pattern using an email template example.
 * 
 * The `EmailTemplate` interface defines the structure for email templates, including a clone method for creating copies of the template.
 * The `WelcomeEmail` class implements the `EmailTemplate` interface and provides a specific implementation for a welcome email.
 * The `EmailTemplateRegistry` class serves as a registry for different email templates, allowing clients to retrieve clones of the templates without modifying the originals.
 * 
 * In the `Main` class, we demonstrate how to use the `EmailTemplateRegistry` to create and send customized welcome emails to different recipients while reusing the same base template structure.
 */